...
@dataclass
class LightCurveItem:
    """Lightweight in-memory light-curve point used by legacy code.

    This is NOT an ORM row. diffphot.py only needs a simple container for
    star_id, image_id, magnitude, and optional snr.
    """
    star_id: int
    image_id: int
    magnitude: float
...
# Legacy alias some versions expect
LightCurve = LightCurveItem
...
class LEMONdB:
    ...
    def commit(self) -> None:
        """
        No-op compatibility method. Older code might call output_db.commit().
        All our methods commit by default unless inside fast_transaction().
        """
        pass
