...
class LEMONJuicerWindow(Gtk.ApplicationWindow):
    ...
    def _open_star_window(self, row: StarRow) -> None:
        ...
        try:
            ...
            x_px = float(getattr(row.props, "x", 0.0))
            y_px = float(getattr(row.props, "y", 0.0))
    ...
    def _populate_overview_from_miner(self, miner) -> None:
        ...
        for star_id in star_ids:
            try:
                ...
                # Extract ra/dec/imag/x/y from various shapes
                ra = dec = imag = x = y = None

                if isinstance(s, (tuple, list)):
                    # Common miner shapes:
                    # (x, y, ra, dec, ..., imag)  -> ra=s[2], dec=s[3], imag=s[-1]
                    if len(s) >= 5:
                        x, y, ra, dec, imag = s[0], s[1], s[2], s[3], s[-1]
