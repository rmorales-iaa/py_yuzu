...
def _scrub_stale_uparm() -> None:
    """Remove stale task parameter files that trigger noisy PyRAF warnings."""
    repo_uparm = Path(__file__).resolve().parent / "uparm" / "imlimexpr.par"
    try:
        if repo_uparm.exists():
            repo_uparm.unlink()
    except OSError:
        pass
