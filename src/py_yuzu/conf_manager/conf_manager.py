...
def _ensure_gtk() -> bool:
    """Lazy-load GTK stack only for GUI code paths."""
    global Gtk, Gdk, Gio, GLib, GdkX11, _GI_LOADED
    if _GI_LOADED:
        return Gtk is not None

    _GI_LOADED = True

    # Headless CLI paths only need config I/O. Skip GTK import entirely unless a
    # real display session is present or the caller explicitly requests it.
    if not (
        os.environ.get("DISPLAY")
        or os.environ.get("WAYLAND_DISPLAY")
        or os.environ.get("PY_YUZU_FORCE_GTK")
    ):
        return False
    ...
    try:
        ...
        from gi.repository import Gtk as _Gtk, Gdk as _Gdk, Gio as _Gio, GLib as _GLib, GdkX11 as _GdkX11  # type: ignore
        Gtk, Gdk, Gio, GLib, GdkX11 = _Gtk, _Gdk, _Gio, _GLib, _GdkX11  # type: ignore
        return True
