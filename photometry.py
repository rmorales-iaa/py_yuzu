...
def _build_db_image(img: fitsimage.FITSImage, options) -> database.Image:
...
def parallel_photometry(args):
    ...
    MP_QUEUE.put((db_image, img_qphot))
...
def main(arguments=None):
    ...
    with database.LEMONdB(output_db_path) as output_db:
        ...
        simg = _build_db_image(sources_img, options)
        ...
        for pf, images in sorted(files.items()):
            ...
            pool = multiprocessing.Pool(int(max(1, options.ncores)))
            ...
            # Store with a single fast transaction if available
