...
def diff_fit(pimages, diffs_dir, header, overlap):
    ...
    if rtn_fit['status'] != '0':
        return None
    return (plus, minus, rtn_fit['a'], rtn_fit['b'], rtn_fit['c'], rtn_fit['xmin'], rtn_fit['ymin'], rtn_fit['xmax'], rtn_fit['ymax'], rtn_fit['xcenter'], rtn_fit['ycenter'], int(rtn_fit['npixel']), rtn_fit['xrms'], rtn_fit['yrms'], rtn_fit['rss'], rtn_fit['boxx'], rtn_fit['boxy'], rtn_fit['boxwidth'], rtn_fit['boxheight'], rtn_fit['boxang'])
...
def main(argv=None):
    ...
    try:
        ...
        # Parallel projection
        ncores = args.ncores or cfg.getint("parallel", "ncores", 16)
        pool = mp.Pool(ncores) if ncores > 1 else None
