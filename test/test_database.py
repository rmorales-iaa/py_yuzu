...
class DBStarTest(unittest.TestCase):
    ...
    def test_time_index(self):
        for _ in range(NITERS):
            id_, pfilter, phot_info, times_indexes = DBStarTest.random_data()
            star = DBStar(id_, pfilter, phot_info, times_indexes)
            for unix_time in times_indexes.keys():
    ...
    def make_subset(cls, star):
        ...
        for index in range(sphot_info.shape[1]):
            magnitude = random.uniform(cls.MIN_MAG, cls.MAX_MAG)
            snr = random.uniform(cls.MIN_SNR, cls.MAX_SNR)
    ...
    def test_make_star(self):
        ...
        for _ in range(NITERS):
            ...
            rows = [phot_info[:, index] for index in range(phot_info.shape[1])]
...
class ImageTest(unittest.TestCase):
    ...
    def random_data(cls):
        ...
        object_ = "".join(random.choice(string.letters) for _ in range(size))
    ...
    def nrandom(cls, size, pfilter=None, simult_prob=0.5):
        ...
        for index, unix_time in enumerate(runix_times(size)):
            ...
            if pfilter:
                ...
                all_utimes = set().union(*used_utimes.values())
...
class LightCurveTest(unittest.TestCase):
    ...
    def random_data(cls, pfilter=None, cstars=None):
        ...
        if cstars:
            ...
            cstars = [random.randint(cls.MIN_ID, cls.MAX_ID) for x in range(size)]
    ...
    def test_amplitude(self):
        ...
        for _ in range(NITERS):
            ...
            magnitudes.sort()
            npoints = random.choice(range(1, len(curve) + 1))
...
class LEMONdBTest(unittest.TestCase):
    ...
    def test_add_and_get_candidate_pparams(self):
        ...
        for _ in range(NITERS):
            with LEMONdB(":memory:") as db:
                ...
                for pfilter, expected in added.items():
    ...
    def test_add_and_get_image(self):
        with LEMONdB(":memory:") as db:
            ...
            assert sum(len(p) for p in added_images.values()) == size

            # Extract the Images from the LEMONdB and check that the returned
            # objects are exactly equal to what we added previously
            for pfilter in added_images.keys():
                for unix_time, input_img in added_images[pfilter].items():
    ...
    def test_add_and_get_image_None_fields(self):
        ...
        def assert_None_raises(db, attr, is_sources_img):
            ...
            with self.assertRaisesRegex(sqlite3.IntegrityError, regexp):
    ...
    def test_airmasses(self):
        with LEMONdB(":memory:") as db:
            ...
            for pfilter, airmasses in airmasses.items():
    ...
    def test_get_phase_diagram(self):
        ...
        with LEMONdB(":memory:") as db:
            ...
            expected_phase = [locals()["epoint%d" % x] for x in range(1, 7)]
            expected_phase.sort()
    ...
    def test_set_get_and_del_metadata(self):
        ...
        with LEMONdB(":memory:") as db:
            ...
            regexp = f""METADATA table has no record '{key}'""
            with self.assertRaisesRegex(AttributeError, regexp):
    ...
    def test_metadata_properties(self):
        ...
        for name, values in metaproperties.items():

            # AttributeError is raised if the property is not set
            with LEMONdB(":memory:") as db1:
                with self.assertRaisesRegex(AttributeError, regexp % name):
                    getattr(db1, name)
