...
def get_dss_image(path, ra, dec):
    ...
    If any exception is raised by urllib.request.urlretrieve() (and this includes
    ...
    status = functools.partial((f""Downloading {path}: {0:d} %%"").format)
    ...
    try:
        ...
        urllib.request.urlretrieve(url, filename=path, reporthook=update_status)
    except:
        try:
            os.unlink(path)
        except:
...
def get_image_path(name):
    ...
    """
    basename = f""{re}.fits"".sub("\s+", "_", name)
