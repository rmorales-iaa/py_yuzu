...
class WeightsTest(unittest.TestCase):
    ...
    def rcoeffs(n):
        ...
        return [random.random() for index in range(n)]
    ...
    def test_rescale(self):
        ...
        for coeffs in self.rcoefficients:
            ...
            index = random.choice(range(len(weights)))
...
class StarSetTest(unittest.TestCase):
    ...
    def rDBStars(self, size=None, nrecords=None, pfilter=None):
        ...
        stars_ids = random.sample(range(*self.IDS_RANGE), size)
    ...
    def test_delitem(self):
        ...
        for _ in range(NITERS):
            stars = self.rDBStars()
            set_ = StarSet(stars)
            rindex = random.choice(range(len(stars)))
            del set_[rindex]

            self.assertEqual(len(set_), len(stars) - 1)
            _eq_ = test_database.DBStarTest.equal
            for index in range(rindex):
                self.assertTrue(_eq_(set_[index], stars[index]))
            for index in range(rindex, len(set_)):
    ...
    def _populate_set(self, magnitudes, snrs=None):
        ...
        if not snrs:
            ...
            for _ in range(len(magnitudes)):
                star_snrs = [random.uniform(*self.SNR_RANGE) for x in range(n)]
        ...
        star_ids = random.sample(range(*self.IDS_RANGE), size)
    ...
    def _assert_light_curve(
        ...
        candidate_ids = set(range(*self.IDS_RANGE)) - set(cmp_stars.star_ids)
