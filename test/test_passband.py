...
class PassbandTest(unittest.TestCase):
    ...
    def test_custom_filters(self):
        for name, description in passband.CUSTOM_FILTERS.items():
    ...
    def test_all(self):
        ...
        for system, letters in Passband.SYSTEM_LETTERS.items():
        ...
        for name in passband.CUSTOM_FILTERS.keys():
            pfilter = Passband(name)
            self.assertIn(pfilter, pfilters)
    ...
    def test_cmp(self):
        ...
        for _ in range(NITERS):
            ...
            pfilters = [Passband.random() for x in range(NPASSBANDS)]
            pfilters.sort()

            for index in range(0, len(pfilters) - 1):
    ...
    def test_load_custom_filters(self):
        ...
        section_header = f""[{passband}]"".CUSTOM_SECTION
        ...
        data = f""\n".join([section_header] + ["{x} = %s"" for x in custom_filters])
